...
<head>
    ...
    @if (isset($isVideoPage) && $isVideoPage)
        <meta name="keywords" content="{{ implode(', ', $video->tags->pluck('tag_name')->toArray()) }}">
        <meta property="og:title" content="{{ $default_title ?? asset('icon.png') }}">
        <meta property="og:description" content="{{ $default_description ?? 'The best free adult content' }}">
        <meta property="og:image" content="{{ $page_thumb ?? asset('icon.png') }}">
        <meta property="og:video" content="{{ $video->url }}">
        <meta name="twitter:card" content="player">
        <meta name="twitter:title" content="{{ $default_title ?? 'Porn Rooster' }}">
        <meta name="twitter:description" content="{{ $default_description ?? 'The best free adult content' }}">
        <meta name="twitter:image" content="{{ $page_thumb ?? asset('icon.png') }}">
        <meta name="twitter:player" content="{{ $video->url }}">
    @else
        <meta name="keywords" content="{{ $default_keywords ?? 'adult, free videos, best content, teen, amauter' }}">
